...
- `pnpm smoke:production` — production-build smoke (`scripts/smoke-production-build.ts`): imports the Vercel adapter's built function entry from `.vercel/output/` and asserts SSR semantics from the actual deployed bundle (everything else in CI runs against `pnpm dev`). Requires a prior `pnpm build`; in CI it runs inside verify-all right after the Lighthouse step's build.
- `pnpm verify:all` — chains all five; must pass before any PR is opened
