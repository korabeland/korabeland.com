...
`index.astro` passes it, `about.astro` does not).
...
layer) rides in a lid **aperture** punched out of an **occluder**, over a
diffusion-inpainted sclera **underlay**, under a fixed **highlight** catchlight.
The four layers per eye per variant are cut from the pristine sources by
`scripts/gen-eye-rig.ts` into `public/portrait/rig/` (gitignored `.gen.*`,
regenerated on demand; ~78KB two-variant set). All geometry and motion live in
`src/lib/portrait-gaze.ts` (the SSOT — iris/aperture/face-zone ellipses, travel
ceiling, the fixation state machine and main-sequence motion), so markup,
generator, and behaviour never drift; a geometry hash stamped into the rig meta
fails CI when assets trail the SSOT. Double-gated (`pointer: fine` **and** not
`prefers-reduced-motion`): the layers are never fetched until the client script
...
scripts, never hand-place files; edit rig geometry in the `portrait-gaze.ts`
SSOT and regenerate, never nudge the generated layers or meta by hand.
